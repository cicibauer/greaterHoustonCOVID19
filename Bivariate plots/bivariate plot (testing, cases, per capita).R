## This file creates a bivariate choropleth map of positive cases of COVID 19 per capita against the number of tests administered per capita per 
#county in Texas. 
...
## Positive cases per capita data 
...
weekcasecountpercap <- weekcasecount/as.numeric(p1$...2[3:256])
#this provides the total number of cases per county per capita in the week of 6/14 - 6/20

## Testing per capita data 
...
weektestcountpercap <- weektestcount/as.numeric(p1$...2[3:256])
# this provides the total number of tests per county per capita in the week of 6/14 - 6/20
...
cols <- stevens.greenblue #changes colors

nbins <- 3  # 3 bins for each variable 
brks_cases <- classIntervals(weekcasecountpercap, n=nbins, style='quantile')
brks_tests <- classIntervals(weektestcountpercap, n=nbins, style='quantile')
class_cases <- findCols(brks_cases)
class_tests <- findCols(brks_tests)
class2percap <- class_cases + nbins*(class_tests-1)
class2percap <- as.numeric(class2percap)
# assigns each county a "bin", bins 1-9, based on their cases, test numbers 

tests_cases_data_percap <- data.frame(counties, weekcasecountpercap, weektestcountpercap, class2percap) 

## Creating map 
m3 <- mapplot(tests_cases_data_percap$counties ~ as.numeric(tests_cases_data_percap$class2percap), data = tests_cases_data_percap,
...
m3leg <- levelplot(matrix(1:(nbins*nbins), nrow=nbins), axes=FALSE, col.regions=cols(),
                   xlab="  cases percap  -->", ylab="  tests percap -->", cuts=8, colorkey=FALSE,
                   scales=list(draw=0))
...
vp <- viewport(x=.5, y=.95, width=.2, height=.2)
pushViewport(vp)
grid.text("Bivariate Plot of Texas Counties' Testing and Positive Cases per Capita from June 14-20")
popViewport()
